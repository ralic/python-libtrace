...
def print_data(msg, offset, data, mxlen):
    ...
    print("%s%s" % (msg, pad), end=' ')  # Trailing comma suppresses the linefeed
...
def print_tcp(tcp, margin):
    ...
    print("          flags=%02x (%s), window=%u, checksum=%x, urg_ptr=%u" % (
        tcp.flags, fl, tcp.window, tcp.checksum, tcp.urg_ptr), end=' ')
    payload = tcp.payload
    if not payload:
        print(", no payload")
    else:
...
def print_icmp(icmp, offset):  # IPv4 only  (IPv6 uses ICMP6 protocol)
    ...
    if type == 0 or type == 8:  # Echo Reply, Echo Request
        ...
        print("%sDestination unreachable," % (margin), end=' ')
        print_icmp_ip(p, margin);  pd = p.data
    elif type == 4:  # Source Quench
        print("%sSource quench," % (margin), end=' ')
        print_icmp_ip(p, margin);  pd = p.data
    elif type == 5:  # Redirect
        redirect = icmp.redirect;
        print("%sRedirect, gateway=%s," % (margin, redirect.gateway), end=' ')
        print_icmp_ip(p, margin);  pd = p.data
    elif type == 11:  # Time Exceeded
        print("%sTime exceeded," % (margin), end=' ')
        print_icmp_ip(p, margin);  pd = p.data
    elif type == 12:  # Parameter problem
        print("%sParameter Probl\em," % (margin), end=' ')
        print_icmp_ip(p, margin);  pd = p.data
    else:
        pt = 'Other'
    t = margin + pt
...
def print_icmp6(icmp6, offset):  # IPv6 only
    ...
    if type == 1:  # Destination Unreachable
        ...
        print("%sPacket Too Big; MTU=%d:" % (margin, icmp6.toobig.mtu), end=' ')
        pt = 'IP  '
        print_ip6_info(p);  pd = p.data
        ...
        print("%sParameter Problem; pointer=%d," % (margin, icmp6.param.pointer), end=' ')
        pt = 'IP6 '
        print_ip6_info(p);  pd = p.data
    else:
        if type == 133:
